...
namespace dumper::writer {
    ...
    // drop-in header for the Skidware SDK: flat, globally-unique names inside
    // namespace SDK::Offsets (paste over Roblox/Offsets.hpp)
    class SdkWriter : public IWriter {
      protected:
    ...
    // paste over Roblox/FFlags.hpp: flag address table + typed accessors that go
    // through SDK::Memory (external read/write of the live session)
    class SdkFFlagsWriter : public IWriter {
      protected:
        auto get_file_extension() -> std::string override { return ".hpp"; }
        auto generate_content() -> std::string override;
        auto generate_header_comment(std::chrono::milliseconds elapsed_time)
    ...
    inline SdkWriter g_sdk_writer;
    inline SdkFFlagsWriter g_sdk_fflags_writer;
} // namespace dumper::writer