...
namespace bridge {
    ...
    auto Bridge::initialize() -> bool {
        // locate ReplicatedStorage via RTTI field scan (child iteration can fail on
        // Studio's multi-datamodel layouts; service pointers live in DataModel fields)
        std::optional<roblox::Instance> replicated_storage;

        const auto rs_off = process::Rtti::find(dumper::g_data_model_addr,
                                                "ReplicatedStorage@RBX", 0x1000);
        if (rs_off) {
            const auto rs_addr =
        ...
        m_workspace_information = find("WorkspaceInformation");
        ...
        m_animations_information = find("AnimationsInformation");
        m_props_information = find("PropsInformation");
    ...
    auto Bridge::read_as(const roblox::Instance& instance, const char* name) -> std::optional<T> {
        ...
        try {
            ...
            spdlog::error("Failed to deserialize {}: {} (len={}) - if keys are missing, the "
                          "place's ControlListener script is outdated vs this dumper build",
                          name, e.what(), j->dump().size());
            return std::nullopt;
        }
    }
    ...
    auto Bridge::read_workspace_information() -> std::optional<WorkspaceInfo> {
        if (!m_workspace_information) {
            spdlog::warn("No WorkspaceInformation StringValue found (using default child count)");
            return std::nullopt;
        }
        return read_as<WorkspaceInfo>(*m_workspace_information, "WorkspaceInformation");
    }
    ...
    auto Bridge::read_props_information() -> std::optional<PropsInfo> {
        if (!m_props_information) {
            return std::nullopt;
        }
        return read_as<PropsInfo>(*m_props_information, "PropsInformation");
    }
    ...
    auto Bridge::read_animations_information() -> std::optional<AnimationPropertiesInfo> {
        if (!m_animations_information) {
            spdlog::warn("No AnimationsInformation StringValue found (skipping animations)");
            return std::nullopt;
        }
        return read_as<AnimationPropertiesInfo>(*m_animations_information,
