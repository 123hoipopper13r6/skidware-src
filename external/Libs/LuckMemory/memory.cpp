...
std::uint32_t memory_t::find_process_id(const std::string& process_name)
{
	...
	#ifdef UNICODE
		wchar_t wname[260];
		MultiByteToWideChar(CP_ACP, 0, process_name.c_str(), -1, wname, 260);
		if (!_wcsicmp(wname, process_entry.szExeFile))
#else
...
std::uint64_t memory_t::find_module_address(const std::string& module_name)
{
	...
	#ifdef UNICODE
		wchar_t mname[260];
		MultiByteToWideChar(CP_ACP, 0, module_name.c_str(), -1, mname, 260);
		if (!_wcsicmp(mname, module_entry.szModule))
#else
